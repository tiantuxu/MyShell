	...
	while (*str != '\0') {
		if (*str == '\"'){
			...
			// Token too long; this is an error
				...
				if(i >= TOKENSIZE - 1) goto error;
				token->buffer[i++] = *str; 
		    		str++;		
			}
		}
	}
	if(quote_state) goto error;
...
command_parse(parsestate_t *parsestate)
{
	...
	while (1) {
		...
		case TOK_LESS_THAN:
			...
			else{cmd->redirect_filename[STDIN_FILENO] = strdup(token.buffer);}
			break;
		case TOK_GREATER_THAN:
			parse_gettoken(parsestate,&token);
			if(token.type != TOK_NORMAL){goto error;}
			else{cmd->redirect_filename[STDOUT_FILENO] = strdup(token.buffer);}
			break;
		case TOK_2_GREATER_THAN:
			parse_gettoken(parsestate,&token);
			if(token.type != TOK_NORMAL){goto error;}
			else{cmd->redirect_filename[STDERR_FILENO] = strdup(token.buffer);}
			break;
		case TOK_OPEN_PAREN:
	...
	while (1) {
		...
		switch (token.type){
			case CMD_SEMICOLON:
				...
				parse_gettoken(parsestate,&token);
				if (token.type == TOK_END) goto done;
				parse_ungettoken(parsestate);
				continue;
			case CMD_BACKGROUND:
			...
			case CMD_AND:
				...
				parse_gettoken(parsestate,&token);
				if (token.type == TOK_END) goto error;
				parse_ungettoken(parsestate);
				continue;
			case CMD_OR:
